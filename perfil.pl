...
%procuraLocalidade(L):- membro('Algarve' , L).

%decisaoLocal(N,L):- (membro(1, N), procuraLocalidade(L)).
...
%verResort([fa], [desportista], [praia, cultura], [sim], [1]).

perfil(1,L):- verResort([fa], [desportista], [natureza, cultura], [sim], L).
